  ...
  const handleScroll = () => {
    ...
    if (roadmapElement) {
      ...
      if (
        ...
        if (scrollOffset >= phaseHeight * 3) {
          newPhaseColors[3] = "bg-primary"; // 4th phase
        }
        if (scrollOffset >= phaseHeight * 2) {
          newPhaseColors[2] = "bg-primary"; // 3rd phase
        }
        if (scrollOffset >= phaseHeight) {
          newPhaseColors[1] = "bg-primary"; // 2nd phase
        }
        newPhaseColors[0] = "bg-primary"; // 1st phase is always blue when active
            ...
            >
              ...
              <ul className="mt-6 flex items-center gap-3 flex-wrap justify-center max-md:flex-col">
                {phase4.map((phase, index) => (
                  <li
                    key={index}
                    className="flex items-center gap-3 max-lg:text-center"
                  >
                    <span className="inline-flex w-1 h-1 rounded-full bg-white"></span>
                    <p>{phase.heading}</p>
                  </li>
          ...
          <div className="roadmap-scale">
            <div className="roadmap-progress-area">
              <div className="roadmap-progress-bar" id="roadmapProgressBar">
                <div
                  className={`roadmap-progress-fill ${phaseColors[0]}`}
                  style={{ height: `${progress}%` }}
                ></div>

                <div
                  className={`w-[70px] h-[70px] rounded-full ${phaseColors[0]} flex justify-center items-center absolute -top-4 -left-8`}
                >
                  <IconUnion />
                </div>

                <div
                  className={`w-[70px] h-[70px] rounded-full ${phaseColors[1]} flex justify-center items-center absolute top-[370px] sm:top-[390px] md:top-[375px] min-[900px]:top-[280px] min-1100:top-[225px] -left-8`}
                >
                  <IconSpeed />
                </div>

                <div
                  className={`w-[70px] h-[70px] rounded-full ${phaseColors[2]} flex justify-center items-center absolute top-[785px] sm:top-[840px] md:top-[775px] min-[900px]:top-[595px] min-[1100px]:top-[504px] min-[1400px]:top-[470px] -left-8`}
                >
                  <IconDatabase />
                </div>

                <div
                  className={`w-[70px] h-[70px] rounded-full ${phaseColors[3]} flex justify-center items-center absolute -bottom-16 -left-8`}
                >
                  <IconUnion />
                </div>
              </div>
